...
class POSWindow(QMainWindow):
    ...
    def __init__(self, current_user):
        ...
        self.order_table.setColumnWidth(1, 45)
        self.order_table.setColumnWidth(2, 95)
    ...
    def _show_func_menu(self):
        ...
        if co_quyen(role, "quan_ly_ca_lam") or co_quyen(role, "xem_nhat_ky") or co_quyen(role, "quan_ly_nhan_su"):
            menu.addSeparator()
    ...
    def filter_products(self, text):
        ...
        for i in range(self.grid_layout.count()):
            ...
            if item and item.widget():
                card = item.widget()
                visible = search_text in card.product.ten_sp.lower()
    ...
    def _rebuild_action_buttons(self):
        """Vẽ lại toàn bộ nút +/- theo row index hiện tại. Bỏ qua row ghi chú."""
        for row in range(self.order_table.rowCount()):
            price_item = self.order_table.item(row, 2)
            qty_item   = self.order_table.item(row, 1)
            # Row ghi chú: col 1 trống
            ...
            total = float(price_item.text().replace(",", ""))
    ...
    def _make_btn_widget(self, row: int, price: float) -> QWidget:
        ...
        style = (
            "font-weight: bold; min-width: 28px; max-width: 28px;"
    ...
    def _change_qty(self, row: int, delta: int, price: float):
        ...
        if not qty_item or not qty_item.text().strip():
            return
        new_qty = int(qty_item.text()) + delta

        # Lấy tên món trước khi xóa
    ...
    def _on_row_double_clicked(self, row: int, col: int):
        """Double-click vào dòng món → mở dialog tùy chỉnh đá/đường/topping/ghi chú."""
    ...
    def handle_checkout(self):
        ...
        while row < self.order_table.rowCount():
            qty_item = self.order_table.item(row, 1)
            # Bỏ qua row ghi chú
            if not qty_item or not qty_item.text().strip() or qty_item.data(Qt.UserRole) == "note_row":
                row += 1
                continue
