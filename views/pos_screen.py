...
class InvoiceTable(QTableWidget):
    ...
    def __init__(self, parent=None):
        ...
        QTableWidget {{
            background-color: {BG_CARD}; border:none; gridline-color: {BORDER};
            color: {TEXT}; font-size: 13px; border-radius: 8px;
        }}
        QTableWidget::item {{ padding: 5px; border-bottom: 1px solid {BORDER}; }}
        QTableWidget::item:selected {{ background-color: {ACCENT}; color: white; }}
        QHeaderView::section {{
            background-color: {BG_PANEL}; color: {TEXT_DIM}; padding: 8px;
            border: none; font-weight: bold; font-size: 12px;
        }}
        """)
    ...
    def _refresh(self):
        ...
        for row, it in enumerate(self._items):
            ...
            btn_m.clicked.connect(lambda *args, r=row: self._dec(r))
            btn_p.clicked.connect(lambda *args, r=row: self._inc(r))
            hl.addWidget(btn_m)
            hl.addWidget(btn_p)
