...
class AttendanceTab(QWidget):
    def __init__(self, parent=None):
        ...
        # ── Toolbar ─────────────────────────────────────────────
        bar = QHBoxLayout()
        bar.addWidget(_label("ĐIỂM DANH THEO NGÀY", "#3498DB", 16, True))
        bar.addStretch()
        bar.addWidget(_label("Ngày:"))
        self.de_ngay = QDateEdit(QDate.currentDate())
        self.de_ngay.setCalendarPopup(True)
        self.de_ngay.setDisplayFormat("dd/MM/yyyy")
        self.de_ngay.setFixedWidth(130)
        bar.addWidget(self.de_ngay)
        btn_load = _btn("🔍 Xem", "#2980B9", 34); bar.addWidget(btn_load)
        root.addLayout(bar)

        # ── Bảng ────────────────────────────────────────────────
        self.table = QTableWidget(0, 8)
        self.table.setHorizontalHeaderLabels([
            "Nhân Viên", "Ca", "Ca Bắt Đầu", "Ca Kết Thúc",
            "Check-In", "Check-Out", "Giờ Làm", "Trạng Thái"
        ])
        hh = self.table.horizontalHeader()
        ...
        # ── Nút hành động ───────────────────────────────────────
        act_bar = QHBoxLayout()
        self.btn_checkin  = _btn("✅ Check-In",  "#27AE60")
        self.btn_checkout = _btn("🚪 Check-Out", "#E67E22")
        self.btn_vang     = _btn("❌ Đánh Vắng", "#C0392B")
        self.lbl_gio      = _label("", "#2ECC71", 13, True)
        for w in [self.btn_checkin, self.btn_checkout, self.btn_vang, self.lbl_gio]:
            act_bar.addWidget(w)
        act_bar.addStretch()
        ...
        self.de_ngay.dateChanged.connect(lambda _: self.load())
        self.btn_checkin.clicked.connect(self._checkin)
        self.btn_checkout.clicked.connect(self._checkout)
        self.btn_vang.clicked.connect(self._mark_absent)
        self.table.itemSelectionChanged.connect(self._update_gio_label)

        self.load()

    # ── Load bảng phân công theo ngày ───────────────────────────
    def load(self):
        self.table.setRowCount(0)
        self.lbl_gio.clear()
        qd = self.de_ngay.date()
        ...
        try:
            pcs = (session.query(PhanCongCaLam)
                   .filter_by(ngay_lam=ngay)
                   .all())
            for i, pc in enumerate(pcs):
                nv  = session.query(NhanVien).get(pc.ma_nv)
                ca  = session.query(CaLamViec).get(pc.ma_ca)
                if not nv or not ca: continue

                # Tìm phiên đăng nhập tương ứng (nếu có)
                phien = (session.query(PhienLamViec)
                         .filter_by(ma_nv=pc.ma_nv, ma_ca=pc.ma_ca)
                         .filter(PhienLamViec.thoi_gian_dang_nhap >= datetime.combine(ngay, dtime.min))
                         .filter(PhienLamViec.thoi_gian_dang_nhap <= datetime.combine(ngay, dtime.max))
                         .order_by(PhienLamViec.thoi_gian_dang_nhap.desc())
                         .first())

                self.table.insertRow(i)

                # Lưu pc.id vào UserRole để thao tác
                id_item = QTableWidgetItem(nv.ten_nv)
                id_item.setData(Qt.UserRole, pc.id)
                id_item.setData(Qt.UserRole + 1, pc.ma_nv)
                id_item.setData(Qt.UserRole + 2, pc.ma_ca)
                self.table.setItem(i, 0, id_item)

                ca_item = QTableWidgetItem(ca.ten_ca)
                ca_item.setForeground(QColor(CA_COLOR.get(ca.ten_ca, "#A1A1AA")))
                self.table.setItem(i, 1, ca_item)
                ...
                # Check-in / out từ PhienLamViec
                if phien and phien.thoi_gian_dang_nhap:
                    ci_str = phien.thoi_gian_dang_nhap.strftime("%H:%M")
                    ci_item = QTableWidgetItem(ci_str)
                    ci_item.setForeground(QColor("#2ECC71"))
                else:
                    ci_item = QTableWidgetItem("—")
                self.table.setItem(i, 4, ci_item)

                if phien and phien.thoi_gian_dang_xuat:
                    co_str = phien.thoi_gian_dang_xuat.strftime("%H:%M")
                    co_item = QTableWidgetItem(co_str)
                    co_item.setForeground(QColor("#E67E22"))
                    # Tính giờ làm
                    delta = phien.thoi_gian_dang_xuat - phien.thoi_gian_dang_nhap
                    gio   = int(delta.total_seconds() // 3600)
                    phut  = int((delta.total_seconds() % 3600) // 60)
                    gio_item = QTableWidgetItem(f"{gio}h{phut:02d}p")
                    gio_item.setForeground(QColor("#F1C40F"))
                else:
                    co_item  = QTableWidgetItem("—")
                    gio_item = QTableWidgetItem("—")
                self.table.setItem(i, 5, co_item)
                self.table.setItem(i, 6, gio_item)
                ...
                tt_item.setForeground(QColor(color_map.get(tt, "#A1A1AA")))
    ...
    def _selected_pc_id(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.information(self, "Chưa chọn", "Hãy chọn một dòng!"); return None, None, None
        item = self.table.item(row, 0)
        return item.data(Qt.UserRole), item.data(Qt.UserRole + 1), item.data(Qt.UserRole + 2)

    def _checkin(self):
        pc_id, ma_nv, ma_ca = self._selected_pc_id()
        if pc_id is None: return
        session = get_session()
        try:
            pc = session.query(PhanCongCaLam).get(pc_id)
            if pc and pc.trang_thai_dd not in ("Đã check-in", "Đã check-out"):
                pc.trang_thai_dd = "Đã check-in"
                # Tạo PhienLamViec
                phien = PhienLamViec(
                    ma_nv=ma_nv, ma_ca=ma_ca,
                    thoi_gian_dang_nhap=datetime.now(),
                    dang_hoat_dong=True
                )
                session.add(phien)
                session.commit()
    ...
    def _checkout(self):
        ...
        try:
            pc = session.query(PhanCongCaLam).get(pc_id)
            if pc and pc.trang_thai_dd == "Đã check-in":
                pc.trang_thai_dd = "Đã check-out"
                phien = (session.query(PhienLamViec)
                         .filter_by(ma_nv=ma_nv, ma_ca=ma_ca, dang_hoat_dong=True)
                         .filter(PhienLamViec.thoi_gian_dang_nhap >=
                                 datetime.combine(ngay, dtime.min))
                         .order_by(PhienLamViec.thoi_gian_dang_nhap.desc())
                         .first())
                if phien:
    ...
    def _mark_absent(self):
        pc_id, _, _ = self._selected_pc_id()
        if pc_id is None: return
        session = get_session()
        try:
            pc = session.query(PhanCongCaLam).get(pc_id)
            if pc: pc.trang_thai_dd = "Vắng"; session.commit()
        finally:
            session.close()
        self.load()

    def _update_gio_label(self):
        row = self.table.currentRow()
        if row < 0: self.lbl_gio.clear(); return
        gio = self.table.item(row, 6)
        tt  = self.table.item(row, 7)
        if gio and tt:
            self.lbl_gio.setText(
                f"  ⏱ Giờ làm: {gio.text()}   |   Trạng thái: {tt.text()}"
            )
...
class ShiftManagerDialog(QDialog):
    ...
    def _on_tab_changed(self, idx: int):
        ...
        elif idx == 2: self.tab_attend.load()
        elif idx == 3: self.tab_week.load()